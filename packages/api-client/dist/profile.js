const isGeneratedCustomerName = (value) => {
...
const hasExactAddressPin = (address) => typeof address?.latitude === "number" &&
    typeof address?.longitude === "number" &&
    Number.isFinite(address.latitude) &&
    Number.isFinite(address.longitude) &&
    !(address.latitude === 0 && address.longitude === 0);
export const isCustomerProfileComplete = (profile) => {
    const hasRealName = !!profile.name && !isGeneratedCustomerName(profile.name) && profile.name.trim().length >= 3;
    const address = profile.address || profile.addresses?.find((entry) => entry.isDefault) || profile.addresses?.[0];
    const hasAddress = !!(address?.houseFlatDoorNo || address?.street) &&
...
export const getShopName = (shop) => shop.shopName || shop.restaurantName || "Shop";
