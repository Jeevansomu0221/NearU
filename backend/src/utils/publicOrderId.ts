...
export const getPublicOrderLastFour = (orderId?: unknown) => getPublicOrderId(orderId).slice(-4);
...
export const formatPublicOrderIdWithLastFour = (orderId?: unknown) => {
  const digits = getPublicOrderId(orderId);
  if (!digits) return "";
  return `#${digits} (last 4: ${digits.slice(-4)})`;
};
